...
mapView(SpatialPointsDataFrame(coords=oneday[,c(2:3)],data= oneday, proj4string=CRS("+init=epsg:4326")))  
...
locationmap <- subset(locations, locations$Longitude<60)
mapView(SpatialPointsDataFrame(coords=locationmap[,c(1:2)],data= locationmap, proj4string=CRS("+init=epsg:4326")))
